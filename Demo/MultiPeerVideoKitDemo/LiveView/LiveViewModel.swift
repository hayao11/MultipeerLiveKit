...
final class LiveViewModel: NSObject {
    ...
    init(targetPeerID: MCPeerID?, mcSessionManager: MCSessionManager
        ,sendVideoInterval:TimeInterval,sessionPreset:AVCaptureSession.Preset = .low) throws {
        self.targetPeerID = targetPeerID
        super.init()
        self.mcSessionManager = mcSessionManager
        try livePresenter = LivePresenter.init(mcSessionManager: mcSessionManager, sendVideoInterval: sendVideoInterval,
                                               targetPeerID: targetPeerID, sessionPreset: sessionPreset)
    ...
    @objc private func cameraToggle(_ sender: UIButton) {
        ...
        do {
            try self.livePresenter.toggleCamera()
        } catch let error {
            print(error)
        }
    ...
    private func setUpButtonLabel(buttonType: ButtonType) -> ButtonDisplayData {
        switch  buttonType {
        case .sound:
            return soundBtnData[!self.needsMute]!
        case .sendVideo:
    ...
    @objc private func sendText() {
        do {
            try livePresenter.send(text: self.sendString, sendMode: .unreliable)
        } catch let error {
            print(error)
        }
    }
